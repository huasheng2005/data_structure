...
struct BST {
	...
	void printPath2(BTNode* curr, stack<BTNode*>&);
	void PrintID2(BTNode*);
	...
	void PrintDescending(BTNode*);
	void fileAscending(BTNode*, ofstream&);
	void fileDescending(BTNode*, ofstream&);
	bool deepestNodes();
	void countNodes(BTNode*, int, int&);
	void printNodes(BTNode*, int, int);
	BTNode* findNode(BTNode*, type);
	bool CloneSubtree(BST, type);
	void CloneSubtree2(BTNode*, BST, type, BST&);
	bool printLevelNode();
	bool printPath();
	void PrintID();
